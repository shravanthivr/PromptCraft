...
const PromptForm = () =>  {
  ...
  const { loading, generatedResponse, error}  = useGenerateAI(selectedTool, question);
    return (
      <>
        <section className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-8 hover:shadow-xl transition-shadow duration-300">
          <label className="block text-xl font-semibold mb-3 bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">
            Prompt for: {selectedTool}
          </label>
          <textarea
          ...
          />
          <button
            className="w-full py-3.5 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 hover:from-blue-600 hover:to-blue-800"
            onClick={() => {
        ...
        <section className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-8 hover:shadow-xl transition-shadow duration-300">
          ...
          <div className="w-full min-h-[120px] border-2 border-blue-100 rounded-xl p-5 bg-white/50 text-gray-700 shadow-inner">
            <span className="text-blue-300/80">{loading ? '[AI-generated text appears here]' :  generatedResponse}</span>
          </div>
        </section>
      </>
    )
